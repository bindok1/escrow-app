...
const PricingCard = ({ price, onPriceChange }: PricingCardProps) => {
    ...
    <Box p={3}>
      ...
      <Grid container spacing={3}>
        <Grid item xs={12}>
          ...
          <CustomTextField 
            id="p_price" 
            placeholder="Product Price by BNB" 
            fullWidth
            value={price}
            onChange={(e: React.ChangeEvent<HTMLInputElement>) => onPriceChange(e.target.value)}
          />
          <Typography sx={{mt:1}} variant="body2">Set the product price by BNB.</Typography>
        </Grid>
        {/* <Grid item xs={12}>
          <CustomFormLabel htmlFor="p_price" sx={{ mt: 0 }}>
            Discount Type{" "}
